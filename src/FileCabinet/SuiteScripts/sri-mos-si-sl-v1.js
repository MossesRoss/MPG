...
function createUI(scriptContext, myParams) {
    ...
    try {
        ...
        if (action === "POST") {
            ...
            var outputGroup = form.addFieldGroup({ id: "custpage_output_group", label: "Simple Interest" });
            var outputGroup1 = form.addFieldGroup({ id: "custpage_output_group1", label: "Compound Interest" });
            var outputGroup2 = form.addFieldGroup({ id: "custpage_output_group2", label: "Compound Interest Table" });
            var outputGroup3 = form.addFieldGroup({ id: "custpage_output_group3", label: "EMI Table" });

            var fldSimpleInterest = form.addField({
                id: "custpage_simple_interest",
                type: serverWidget.FieldType.CURRENCY,
                label: "Simple Interest",
                container: "custpage_output_group",
            ...
            var fldCompoundInterest = form.addField({
                ...
                container: "custpage_output_group1",
            }).updateDisplayType({ displayType: serverWidget.FieldDisplayType.DISABLED });
            ...
            var fldCompoundInterestTable = form.addField({
                ...
                container: "custpage_output_group2",
            ...
            var fldEmiTable = form.addField({
                ...
                type: serverWidget.FieldType.INLINEHTML,
                label: "EMI Table",
                container: "custpage_output_group3",
            }).updateDisplayType({ displayType: serverWidget.FieldDisplayType.INLINE });
...
function processPost(scriptContext) {
    ...
    try {
        ...
        var compoundInterest = ctsUtils.calculateCompoundInterest(
            Principal,
            Rate,
            Time
        );
...
function generatePdfFileFromRawXml(Principal, Rate, Time, simpleInterest, compoundInterest, compoundInterestTableHtml, emiTableHtml)
    ...
    try {
        ...
        pdfFile.name = "CalculationResults.pdf";
        return pdfFile;
    } catch (e) {
        log.error({ title: "Error Generating PDF", details: e.message });
        throw e;
